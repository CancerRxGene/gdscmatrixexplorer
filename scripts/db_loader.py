...
def get_sidm(cosmic_id: int):
    resp = requests.get(f"https://api.cellmodelpassports.sanger.ac.uk/models/COSMIC_ID/{cosmic_id}?fields[model]=id")
...
def extract_well_results(combo_well_stats):
    ...
    columns = ['DRUGSET_ID', 'cmatrix', 'BARCODE', 'POSITION', 'lib1_dose',
               'lib2_dose', 'combo_viability', 'HSA', 'HSA_excess',
               'Bliss_additivity', 'Bliss_index', 'Bliss_excess',
