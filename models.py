...
class AnchorCombi(ToDictMixin, Base):
    ...
    library_id = sa.Column(sa.Integer, sa.ForeignKey(Drug.id), nullable=False, primary_key=True, index=True)
    barcode = sa.Column(sa.Integer)
    scan_id = sa.Column(sa.Integer,primary_key=True)
    ...
    scal = sa.Column(sa.Float)
    anchor_id = sa.Column(sa.Integer, sa.ForeignKey(Drug.id), nullable=False, primary_key=True, index=True)
    anchor_conc = sa.Column(sa.String, primary_key=True,)
    ...
    sidm = sa.Column(sa.String, sa.ForeignKey(Model.id), nullable=False, primary_key=True, index=True)
